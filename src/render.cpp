...
void dibujar_cuerpos(std::vector<uint32_t>& fb, int ancho, int alto,
    ...
    for (int i = 0; i < n; ++i) {
        ...
        // Descarte temprano: lo que quedo fuera del canvas no se dibuja.
        if (cx < -RADIO_DESTELLO || cx >= ancho + RADIO_DESTELLO ||
            cy < -RADIO_DESTELLO || cy >= alto  + RADIO_DESTELLO) continue;

        const float rapidez = std::sqrt(vx[i] * vx[i] + vy[i] * vy[i]);
        const ColorRGB c = color_por_rapidez(rapidez * inv_ref);

        const int y0 = std::max(0, cy - RADIO_DESTELLO);
        const int y1 = std::min(alto  - 1, cy + RADIO_DESTELLO);
